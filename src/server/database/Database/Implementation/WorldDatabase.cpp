...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_CREATURE_TEMPLATE, "SELECT entry, difficulty_entry_1, difficulty_entry_2, difficulty_entry_3, KillCredit1, KillCredit2, modelid1, modelid2, modelid3, modelid4, name, subname, IconName, gossip_menu_id, minlevel, maxlevel, exp, faction, npcflag, speed_walk, speed_run, scale, rank, dmgschool, BaseAttackTime, RangeAttackTime, BaseVariance, RangeVariance, unit_class, unit_flags, unit_flags2, dynamicflags, family, trainer_type, trainer_spell, trainer_class, trainer_race, type, type_flags, lootid, pickpocketloot, skinloot, resistance1, resistance2, resistance3, resistance4, resistance5, resistance6, spell1, spell2, spell3, spell4, spell5, spell6, spell7, spell8, PetSpellDataId, VehicleId, mingold, maxgold, AIName, MovementType, InhabitType, HoverHeight, HealthModifier, ManaModifier, ArmorModifier, DamageModifier, ExperienceModifier, RacialLeader, movementId, RegenHealth, mechanic_immune_mask, flags_extra, ScriptName,aspell1,aspell2, aspell3, aspell4, aspell5, aspell6, aspell7, aspell8, aspell9, aspell10, aspell1, aspell12 FROM creature_template WHERE entry = ?", CONNECTION_SYNCH);
	...
	PrepareStatement(WORLD_SEL_NPCBOT_ID, "SELECT entry, trainer_class FROM creature_template WHERE name = ? AND trainer_race IN (?, ?, ?, ?, ?) AND entry >= 70001 AND entry <= 70100", CONNECTION_SYNCH);
	PrepareStatement(WORLD_SEL_NPCBOT_TEMPLATE_BYID, "SELECT entry, trainer_race, trainer_class FROM creature_template WHERE entry = ? AND scriptname LIKE CONCAT('%%', '_bot') AND trainer_race IN (?, ?, ?, ?, ?)", CONNECTION_SYNCH);

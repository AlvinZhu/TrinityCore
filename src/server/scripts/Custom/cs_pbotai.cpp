...
static bool HandlePBotAIABFlag(ChatHandler* handler, const char* args)
{
	...
	if (pFlag)
	{
		...
		pSelf->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_USE_GAMEOBJECT, pFlag->GetEntry());
	}
	else
		return false;
	return true;
...
static bool HandlePBotAIECPInfo(ChatHandler* handler, const char* args)
{
	...
	float critMeleeTaken = pSelect->GetRatingBonusValue(CR_CRIT_TAKEN_MELEE);
	float critRangeTaken = pSelect->GetRatingBonusValue(CR_CRIT_TAKEN_RANGED);
	float critSpellTaken = pSelect->GetRatingBonusValue(CR_CRIT_TAKEN_SPELL);
	char text[128] = { 0 };
	if (ecpGap == 0)
	{

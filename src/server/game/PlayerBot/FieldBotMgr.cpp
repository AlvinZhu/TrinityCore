...
bool FieldBotMgr::MatchTeleportCondition(Player* p1, Player* p2)
{
	...
	if (zone)
	{
		if (zone->ID == 876) // is gm island
			return false;
		if ((zone->flags & AREA_FLAG_TOWN) && (zone->flags & AREA_FLAG_CAPITAL) && (zone->flags & AREA_FLAG_SLAVE_CAPITAL2) && (zone->flags & AREA_FLAG_SLAVE_CAPITAL))
		{
...
void FieldBotMgr::TeleportToTargetVisible(BotFieldAI* pFieldAI, Player* pTarget)
{
	...
	if (zone && (zone->flags & AREA_FLAG_TOWN) && (zone->flags & AREA_FLAG_CAPITAL) && (zone->flags & AREA_FLAG_SLAVE_CAPITAL2) && (zone->flags & AREA_FLAG_SLAVE_CAPITAL))
	{

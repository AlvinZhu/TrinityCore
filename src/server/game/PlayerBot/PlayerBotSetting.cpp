...
bool ItemsForLevel::IsTenacityItem(const ItemTemplate* itemTemplate)
{
	...
	{
		const _ItemStat& is = itemTemplate->ItemStat[i];
		if (is.ItemStatType == ItemModType::ITEM_MOD_CRIT_TAKEN_RATING ||
			is.ItemStatType == ItemModType::ITEM_MOD_RESILIENCE_RATING)
		{
			if (itemTemplate->Quality < ItemQualities::ITEM_QUALITY_EPIC)
				continue;
			if (is.ItemStatValue >= 40)
				return true;
			if (itemTemplate->InventoryType == InventoryType::INVTYPE_THROWN ||
				itemTemplate->InventoryType == InventoryType::INVTYPE_RANGED ||
				itemTemplate->InventoryType == InventoryType::INVTYPE_RANGEDRIGHT)
			{
				if (is.ItemStatValue >= 20)
					return true;
			}
			if (itemTemplate->InventoryType == InventoryType::INVTYPE_TRINKET)
				return true;
		}
	}
	return false;
...
bool PlayerBotSetting::IsEquipByClsAndTal(uint32 cls, uint32 tal, const ItemTemplate* itemTemplate, int32 rndPropID)
{
	...
	case 1:
		if (tal == 2)
		{
			if (itemTemplate->Class == ItemClass::ITEM_CLASS_WEAPON && itemTemplate->InventoryType == INVTYPE_2HWEAPON)
				return false;
...
bool PlayerBotSetting::MatchEquipmentSlot(uint8 pos, const ItemTemplate* itemTemplate)
{
	...
	InventoryType type = InventoryType(itemTemplate->InventoryType);
	switch (type)
	{
...
{
	if (!itemTemplate || itemTemplate->RequiredLevel > player->getLevel())
		return false;
	if (!IsEquipByClsAndTal(player->getClass(), player->FindTalentType(), itemTemplate, rndPropID))
		return false;
	uint16 eDest;
	InventoryResult msg = player->CanEquipNewItem(NULL_SLOT, eDest, itemTemplate->ItemId, true);
	if (msg != EQUIP_ERR_OK)
		return false;
	...
	for (uint8 slot = EquipmentSlots::EQUIPMENT_SLOT_HEAD; slot < EquipmentSlots::EQUIPMENT_SLOT_END; slot++)
	{
		...
		if (slot == EquipmentSlots::EQUIPMENT_SLOT_MAINHAND)
		{
			if (selfTemplate->Class == ItemClass::ITEM_CLASS_WEAPON && selfTemplate->InventoryType == INVTYPE_2HWEAPON)
				mainhandIsTwo = true;
			else
				mainhandIsTwo = false;
		}
		if (selfTemplate->Class != itemTemplate->Class)
			continue;
		if (selfTemplate->SubClass != itemTemplate->SubClass)
		{
			if (selfTemplate->Class != ItemClass::ITEM_CLASS_WEAPON || itemTemplate->Class != ItemClass::ITEM_CLASS_WEAPON)
				continue;
			if ((selfTemplate->InventoryType != INVTYPE_2HWEAPON && selfTemplate->InventoryType != INVTYPE_WEAPON && selfTemplate->InventoryType != INVTYPE_WEAPONMAINHAND) ||
				(itemTemplate->InventoryType != INVTYPE_2HWEAPON && itemTemplate->InventoryType != INVTYPE_WEAPON && itemTemplate->InventoryType != INVTYPE_WEAPONMAINHAND))
				continue;
		}
...
{
	if (itemTemplate->StatsCount == 0)
		return false;
	for (uint32 i = 0; i < itemTemplate->StatsCount; i++)
	{
		_ItemStat stat = itemTemplate->ItemStat[i];
...
void PlayerBotSetting::GetRandomPropEnchantments(int32 rndPropID, std::list<uint32>& enchants)
{
	...
	if (rndPropID > 0)
	{
		if (ItemRandomPropertiesEntry const* item_rand = sItemRandomPropertiesStore.LookupEntry(rndPropID))
		{
			...
			{
				uint32 enchant_id = item_rand->enchant_id[i - PROP_ENCHANTMENT_SLOT_2];
				if (enchant_id > 0)
	...
	if (rndPropID < 0)
	{
		if (ItemRandomSuffixEntry const* item_rand = sItemRandomSuffixStore.LookupEntry(-rndPropID))
		{
			...
			{
				uint32 enchant_id = item_rand->enchant_id[i - PROP_ENCHANTMENT_SLOT_0];
				if (enchant_id > 0)
					enchants.push_back(enchant_id);
			}
		}
...
bool PlayerBotSetting::IsCommonEquip(const ItemTemplate* itemTemplate)
{
	...
	if (itemTemplate->Class == ItemClass::ITEM_CLASS_ARMOR && itemTemplate->SubClass == ItemSubclassArmor::ITEM_SUBCLASS_ARMOR_MISC && itemTemplate->InventoryType == INVTYPE_TRINKET)
		return true;
	return false;
}
...
bool PlayerBotSetting::IsFingerEquip(const ItemTemplate* itemTemplate)
{
	...
	if (itemTemplate->Class == ItemClass::ITEM_CLASS_ARMOR && itemTemplate->SubClass == ItemSubclassArmor::ITEM_SUBCLASS_ARMOR_MISC && itemTemplate->InventoryType == INVTYPE_FINGER)
		return true;
	return false;
}
...
bool PlayerBotSetting::IsWarriorEquip(const ItemTemplate* itemTemplate)
{
	...
	else if (itemTemplate->Class == ItemClass::ITEM_CLASS_ARMOR)
	{
		switch (itemTemplate->SubClass)
		{
		...
		case ItemSubclassArmor::ITEM_SUBCLASS_ARMOR_MAIL:
			if (itemTemplate->RequiredLevel >= 40)
				return false;
			break;
		default:
			break;
	...
	else if (itemTemplate->Class == ItemClass::ITEM_CLASS_PROJECTILE)
	{
		if (itemTemplate->SubClass == ItemSubclassProjectile::ITEM_SUBCLASS_ARROW || itemTemplate->SubClass == ItemSubclassProjectile::ITEM_SUBCLASS_BULLET)
			return true;
	}
...
void PlayerBotSetting::Initialize()
{
	...
	for (ItemTemplateContainer::const_iterator itr = its->begin(); itr != its->end(); ++itr)
	{
		...
		if (item.RequiredSkill > 0 || item.RequiredSpell > 0)// || item.AllowableRace != -1)
			continue;
	...
	//hxsd
		...
		if (item.Flags2 > 0)
			continue;
		if (item.RequiredLevel <= 0 || item.InventoryType <= InventoryType::INVTYPE_NON_EQUIP || item.InventoryType > InventoryType::INVTYPE_RELIC)
			continue;
		if (item.RequiredLevel < 20 && item.Quality <= 1 && item.InventoryType != InventoryType::INVTYPE_AMMO)
			continue;
		for (int i = 1; i < MAX_CLASSES; i++)
		{
			...
			{
				InventoryType iType = InventoryType((item.InventoryType == InventoryType::INVTYPE_ROBE) ? (InventoryType::INVTYPE_CHEST) : item.InventoryType);
				BotEquips& equips = classesEquips[i][iType];
				BotEquips::iterator itEquip = equips.find(item.RequiredLevel);
				ItemsForLevel& forLevel = (itEquip == equips.end()) ? equips[item.RequiredLevel] : itEquip->second;
...
uint32 PlayerBotSetting::UpdateTalentType()
{
	if (m_Player->IsPlayerBot())
	{
		...
		uint32 freeTalentPoint = m_Player->GetFreeTalentPoints();
		if (freeTalentPoint >= lv - 9)
		{
			m_ActiveTalentType = 3;
			LearnTalents();
			LearnSpells();
		...
		else if (freeTalentPoint > 6 && !m_Player->InBattleground())
		{
			LearnTalents();
			LearnSpells();
			m_Player->SaveToDB();
...
{
	if (!m_Player || talent > 3)
		return m_ActiveTalentType;
	if (talent < 3 && m_ActiveTalentType == talent && m_Player->GetFreeTalentPoints() <= 0)
		return m_ActiveTalentType;
	if (m_ActiveTalentType != talent)
...
{
	if (m_Player->getClass() != Classes::CLASS_HUNTER)
		return;
	uint32 ammo = m_Player->GetUInt32Value(PLAYER_AMMO_ID);
	if (ammo != 0)
	{
		if (m_Player->HasItemCount(ammo, 1000))
			return;
	}

	Item* pItem = m_Player->GetItemByPos(255, EquipmentSlots::EQUIPMENT_SLOT_RANGED);
	if (!pItem)
		return;
	ItemTemplate* item = (ItemTemplate*)pItem->GetTemplate();
	if (item && item->Class == ItemClass::ITEM_CLASS_WEAPON)
	{
		switch (item->SubClass)
		{
		case ItemSubclassWeapon::ITEM_SUBCLASS_WEAPON_BOW:
		case ItemSubclassWeapon::ITEM_SUBCLASS_WEAPON_CROSSBOW:
			item = (ItemTemplate*)GetRandomAmmoByType(ItemSubclassProjectile::ITEM_SUBCLASS_ARROW, m_Player->getLevel());
			break;
		case ItemSubclassWeapon::ITEM_SUBCLASS_WEAPON_GUN:
			item = (ItemTemplate*)GetRandomAmmoByType(ItemSubclassProjectile::ITEM_SUBCLASS_BULLET, m_Player->getLevel());
			break;
		default:
			return;
		}
	...
	InventoryResult msg = m_Player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, item->ItemId, count, &noSpaceForCount);
	if (msg != EQUIP_ERR_OK)
		count -= noSpaceForCount;
	if (count <= 0 || dest.empty())
		return;
	Item* itemInst = m_Player->StoreNewItem(dest, item->ItemId, true, Item::GenerateItemRandomPropertyId(item->ItemId));
	if (itemInst)
...
void PlayerBotSetting::UpdateReset()
{
	...
	{
		BotGlobleSchedule schedule(BotGlobleScheduleType::BGSType_DelayLevelup, m_Player->GetGUID().GetRawValue());
...
{
	if (m_Player->GetFreeTalentPoints() <= 0)
		return;
	uint32 page = (m_ActiveTalentType > 2) ? urand(0, 2) : m_ActiveTalentType;
	BotTalentPage& botPage = classesTalents[m_Player->getClass()][page];
	for (BotTalentPage::iterator itTalent = botPage.begin();
		itTalent != botPage.end();
		itTalent++)
	{
		const BotTalentEntry& talentEntrys = *itTalent;
		m_ActiveTalentType = talentEntrys.pageIndex;
		for (int i = 0; i < MAX_TALENT_RANK; i++)
		{
			uint32 talID = talentEntrys.talentEntry->RankID[i];
			if (talID != 0)
			{
				if (m_Player->HasSpell(talID))
					continue;
				m_Player->LearnTalent(talentEntrys.talentEntry->TalentID, i);
				if (m_Player->GetFreeTalentPoints() <= 0)
				{
					if (WorldSession* pSession = m_Player->GetSession())
						sOnlineMgr->CharaterState(pSession->GetAccountId(), uint32(m_Player->GetGUID().GetRawValue()), m_Player->getLevel(), uint8(page));
					return;
				}
			}
		}
...
void PlayerBotSetting::UpequipFromAll()
{
		...
		itNeed++)
	{
		...
		if (itemInst->GetTemplate()->InventoryType == InventoryType::INVTYPE_AMMO)
			m_Player->SetAmmo(itemInst->GetEntry());
		else
			EquipItem(itemInst);
	}
...
void PlayerBotSetting::AddOnceEquip(const ItemTemplate* item)
{
	if (!item)
		return;
	//m_Player->AddItem(item->ItemId, 1);
	uint32 count = 1;
	if (item->Class == ItemClass::ITEM_CLASS_PROJECTILE && item->InventoryType == InventoryType::INVTYPE_AMMO)
		count = 1000;
	uint32 noSpaceForCount = 0;
	ItemPosCountVec dest;
	InventoryResult msg = m_Player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, item->ItemId, count, &noSpaceForCount);

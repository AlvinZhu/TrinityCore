...
std::string PlayerBotTalkMgr::GetDefaultChannelName()
{
	...
	consoleToUtf8(std::string("ÊÀ½çÆµµÀ"), defaultChannelName);
...
void PlayerBotTalkMgr::NormalTalk(uint32 sType, Player* player, std::string text)
{
	...
	case 3:
		...
		{
			WorldPacket data;
			ChatHandler::BuildChatPacket(data, ChatMsg::CHAT_MSG_PARTY, Language::LANG_UNIVERSAL, player, NULL, text.c_str());
			pGroup->BroadcastPacket(&data, false, pGroup->GetMemberGroup(player->GetGUID()));
		}

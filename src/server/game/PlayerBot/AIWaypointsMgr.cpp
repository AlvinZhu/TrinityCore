...
void AIWaypoint::NewToDatabase()
{
	...
	PreparedStatement* stmt = WorldDatabase.GetPreparedStatement(WORLD_INS_AIWAYPOINTS);
...
void AIWaypoint::SaveToDatabase()
{
	...
	PreparedStatement* stmt = WorldDatabase.GetPreparedStatement(WORLD_UPD_AIWAYPOINTS);
...
{
	PreparedStatement* stmt = WorldDatabase.GetPreparedStatement(WORLD_DEL_AIWAYPOINTS);
	stmt->setUInt32(0, entry);
	/*PreparedQueryResultFuture resultFuture = */WorldDatabase.AsyncQuery(stmt);
}
...
bool AIWaypointsMgr::LoadAIWaypoints()
{
	...
	PreparedStatement* stmt = WorldDatabase.GetPreparedStatement(WORLD_SEL_ALL_AIWAYPOINTS);

...
uint8 PlayerBotMgr::RandomSkinColor(uint8 race, uint8 gender, uint8 prof)
{
	...
	{
		skinColor = irand(0, 9);
		CharSectionsEntry const* entry = GetCharSectionEntry(race, SECTION_TYPE_SKIN, gender, 0, skinColor);
		if (entry)
		{
...
uint8 PlayerBotMgr::RandomFace(uint8 race, uint8 gender, uint8 skinColor, uint8 prof)
{
	...
	{
		faceID = irand(0, 9);
		CharSectionsEntry const* entry = GetCharSectionEntry(race, SECTION_TYPE_FACE, gender, faceID, skinColor);
		if (entry)
		{
			if ((entry->Flags & SECTION_FLAG_DEATH_KNIGHT) && prof != CLASS_DEATH_KNIGHT)
				continue;
...
uint8 PlayerBotMgr::RandomHair(uint8 race, uint8 gender, uint8 prof)
{
	...
	for (uint8 hc = 0; hc < 80; hc++)
	{
		...
		{
			hairID = irand(0, 8);
			CharSectionsEntry const* entry = GetCharSectionEntry(race, SECTION_TYPE_HAIR, gender, hairID, hc);
			if (entry)
			{
				if ((entry->Flags & SECTION_FLAG_DEATH_KNIGHT) && prof != CLASS_DEATH_KNIGHT)
					continue;
...
uint8 PlayerBotMgr::RandomHairColor(uint8 race, uint8 gender, uint8 hairID, uint8 prof)
{
	...
	{
		hairColor = irand(0, 8);
		CharSectionsEntry const* entry = GetCharSectionEntry(race, SECTION_TYPE_HAIR, gender, hairID, hairColor);
		if (entry)
		{
			if ((entry->Flags & SECTION_FLAG_DEATH_KNIGHT) && prof != CLASS_DEATH_KNIGHT)
				continue;
...
uint8 PlayerBotMgr::RandomFacialHair(uint8 race, uint8 gender, uint8 hairColor, uint8 prof)
{
	...
	for (int i = 0; i < 80; i++)
	{
		...
		CharSectionsEntry const* entry = GetCharSectionEntry(race, SECTION_TYPE_FACIAL_HAIR, gender, facialHair, hairColor);
		if (entry)
		{
			if ((entry->Flags & SECTION_FLAG_DEATH_KNIGHT) && prof != CLASS_DEATH_KNIGHT)
				continue;
	...
	do
	{
		...
		if (pInfo)
		{
			if (pInfo->characters.find(guid) == pInfo->characters.end())
			{
				...
				pInfo->characters[guid] = PlayerBotCharBaseInfo(guid, accID, charName, race, pro, gender, level);
				DestroyBotMail(guid);
			}
		}
...
PlayerBotSession* PlayerBotMgr::UpPlayerBotSessionByBaseInfo(PlayerBotBaseInfo *pAcc, bool accountInfo)
{
	...
	PlayerBotSession* pBotSession = new PlayerBotSession(pAcc->id, name, AccountTypes::SEC_GAMEMASTER, 2, 0, LocaleConstant::LOCALE_zhTW, 0, false);
...
void PlayerBotMgr::OnPlayerBotLogin(WorldSession* pSession, Player* pPlayer)
{
	...
	if (pPlayer->GetBattleground())
	{
		...
		if (pSession)
			pSession->HandleMoveWorldportAckOpcode();
	}
...
void PlayerBotMgr::OnRealPlayerLeaveArenaQueue(uint32 bgTypeId, uint32 level, uint32 aaType)
{
	...
	for (SessionMap::const_iterator itSession = allSession.begin(); itSession != allSession.end(); itSession++)
	{
		...
		if (player->InBattlegroundQueue())
		{
			...
			schedule1.parameter1 = bgTypeId;
			schedule1.parameter2 = bracketEntry->bracketId;
...
if ((allianceCount+hordeCount) >isoka) return false;
...
int32 isoka = sConfigMgr->GetIntDefault("pbotasl", 100);
...
void PlayerBotMgr::QueryBattlegroundRequirement()
{
	for (BattleGroundTypes::iterator itType = m_BGTypes.begin(); itType != m_BGTypes.end(); itType++)
	{
		...
		for (uint32 j = BattlegroundBracketId::BG_BRACKET_ID_FIRST; j <= BattlegroundBracketId::BG_BRACKET_ID_LAST; j++)
		{
			...
			PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketById(bg_template->GetMapId(), bracket);
			if (!bracketEntry)
				continue;
			if (!bgQueue.ExistRealPlayer(bracketEntry))
				continue;
...
void PlayerBotMgr::QueryNonRatedArenaRequirement()
{
	...
	for (BGFreeSlotQueueContainer::iterator itr = bgFreeSlotQueues.begin(); itr != bgFreeSlotQueues.end(); itr++)
	{
		...
		if (bg->GetStatus() > STATUS_WAIT_QUEUE && bg->GetStatus() < STATUS_WAIT_LEAVE)
		{
			if (bg->GetFreeSlotsForTeam(Team::ALLIANCE) > 0)
				AddNewPlayerBotToAA(TeamId::TEAM_ALLIANCE, BattlegroundTypeId::BATTLEGROUND_AA, bracketEntry->bracketId, bg->GetMaxPlayers() / 2);
			else if (bg->GetFreeSlotsForTeam(Team::HORDE) > 0)
				AddNewPlayerBotToAA(TeamId::TEAM_HORDE, BattlegroundTypeId::BATTLEGROUND_AA, bracketEntry->bracketId, bg->GetMaxPlayers() / 2);
			else
				continue;
			return;
		}
	...
	for (uint32 i = 0; i < 3; i++)
	{
		...
		{
			if (needAlliancePlayerCount > 0 && needAlliancePlayerCount >= needHordePlayerCount)
				AddNewPlayerBotToAA(TeamId::TEAM_ALLIANCE, BattlegroundTypeId::BATTLEGROUND_AA, bracketEntry->bracketId, arenaType[i]);
			else if (needHordePlayerCount > 0)
				AddNewPlayerBotToAA(TeamId::TEAM_HORDE, BattlegroundTypeId::BATTLEGROUND_AA, bracketEntry->bracketId, arenaType[i]);
			else
				continue;
			return;
		}

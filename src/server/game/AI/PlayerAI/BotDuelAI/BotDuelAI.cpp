...
Unit* BotDuelAI::GetDuelEnemy()
{
	...
	if (!duel || duel->isCompleted)
	{
		return NULL;
	}
	Unit* pSelect = duel->opponent;
	if (!pSelect)
		return NULL;
...
Position BotDuelAI::GetDuelCenterPosition()
{
	...
	ObjectGuid duelFlagGUID = me->GetGuidValue(PLAYER_DUEL_ARBITER);
...
void BotDuelAI::UpdateBotAI(uint32 diff)
{
	...
	if (pSelect)
	{
		if (me->GetTarget() != pSelect->GetGUID())
...
{
	if (!me->duel || me->duel->isCompleted || !me->duel->opponent || !me->duel->initiator)
	{
		m_Movement->ClearMovement();
		me->SetSelection(ObjectGuid::Empty);
		me->AttackStop();
...
{
	if (!me->duel || me->duel->isCompleted || !me->duel->opponent)
		return false;
	if (me->duel->startTime == 0 || me->duel->startTimer > 0)
		return false;

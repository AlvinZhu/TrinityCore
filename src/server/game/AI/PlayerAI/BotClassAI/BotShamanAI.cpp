...
void BotShamanAI::InitializeSpells()
{
	...
	ShamanIDLE_IceWeapon = FindMaxRankSpellByExist(58796);
	ShamanIDLE_FireWeapon = FindMaxRankSpellByExist(58790);
	ShamanIDLE_PhyWeapon = FindMaxRankSpellByExist(10399);
	ShamanIDLE_FastWeapon = FindMaxRankSpellByExist(58804);

	ShamanShield_Earth = FindMaxRankSpellByExist(49284);
	ShamanShield_Water = FindMaxRankSpellByExist(57960);
	ShamanShield_Lightning = FindMaxRankSpellByExist(49281);

	ShamanAssist_Frog = FindMaxRankSpellByExist(51514);
	ShamanAssist_HealCrit = FindMaxRankSpellByExist(55198);
	ShamanAssist_MomentHeal = FindMaxRankSpellByExist(16188);
	ShamanAssist_MomentCast = FindMaxRankSpellByExist(16166);
	ShamanAssist_BlockCast = FindMaxRankSpellByExist(57994);
	ShamanAssist_Cleansing = FindMaxRankSpellByExist(8012);
	ShamanAssist_FireNova = FindMaxRankSpellByExist(61657);
	...
	ShamanCast_LightningArrow = FindMaxRankSpellByExist(49238);
	ShamanCast_LightningChain = FindMaxRankSpellByExist(49271);
	ShamanCast_LightningStorm = FindMaxRankSpellByExist(59159);
	ShamanCast_FireThud = FindMaxRankSpellByExist(49233);
	ShamanCast_IceThud = FindMaxRankSpellByExist(49236);
	ShamanCast_EarthThud = FindMaxRankSpellByExist(49231);
	ShamanCast_FireStrike = FindMaxRankSpellByExist(60043);

	ShamanHealth_Fast = FindMaxRankSpellByExist(49276);
	ShamanHealth_Bast = FindMaxRankSpellByExist(49273);
	ShamanHealth_Chain = FindMaxRankSpellByExist(55459);
	ShamanHealth_Torrent = FindMaxRankSpellByExist(61301);
	ShamanDispel_Refine = FindMaxRankSpellByExist(51886);

	ShamanTotem_Recycle = FindMaxRankSpellByExist(36936);

	ShamanTotem_Life = FindMaxRankSpellByExist(58757);
	ShamanTotem_Mana = FindMaxRankSpellByExist(58774);
	ShamanTotem_BMana = FindMaxRankSpellByExist(16190);

	ShamanTotem_SummonFire = FindMaxRankSpellByExist(2894);
	ShamanTotem_MgcPower = FindMaxRankSpellByExist(57722);
	ShamanTotem_Attack = FindMaxRankSpellByExist(58704);
	ShamanTotem_AOEAttack = FindMaxRankSpellByExist(58734);
	ShamanTotem_MgcHeal = FindMaxRankSpellByExist(58656);

	ShamanTotem_DecMove = FindMaxRankSpellByExist(2484);
	ShamanTotem_SummonSoil = FindMaxRankSpellByExist(2062);
	ShamanTotem_PhyPower = FindMaxRankSpellByExist(58643);
	ShamanTotem_Armor = FindMaxRankSpellByExist(58753);

	ShamanTotem_AbsorbBuff = FindMaxRankSpellByExist(8177);
	ShamanTotem_AttSpeed = FindMaxRankSpellByExist(8512);
...
bool BotShamanAI::ProcessUpWeapon()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(weaponSpellEntry);
	if (!spellInfo || spellInfo->IsPassive())
		return false;
	Dismount();
	Spell* spell = new Spell(me, spellInfo, TriggerCastFlags::TRIGGERED_IGNORE_CAST_ITEM, ObjectGuid::Empty);
...
bool BotShamanAI::TryStartControlCommand()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(ShamanAssist_Frog);
	if (!spellInfo || spellInfo->IsPassive())
	{
		m_CruxControlTarget = ObjectGuid::Empty;
		return false;
...
bool BotShamanAI::ExistTotemByType(PlayerTotemType totemType, float range)
{
	...
	for (Creature* pObject : results)
	{
		...
		if (!pTotem || pTotem->GetOwner() != me)
			continue;
		if(pTotem->m_Properties->Slot != slotType)
			continue;
		return true;
	}
	return false;

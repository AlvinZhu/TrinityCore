...
{
	PaladinIDLE_MountAura = FindMaxRankSpellByExist(32223);
	PaladinIDLE_CastAura = FindMaxRankSpellByExist(19746);
	PaladinIDLE_JudgeAura = FindMaxRankSpellByExist(54043);
	PaladinIDLE_ArmorAura = FindMaxRankSpellByExist(48942);
	PaladinIDLE_AOEGuardWish = FindMaxRankSpellByExist(25899);
	PaladinIDLE_AOEKingWish = FindMaxRankSpellByExist(25898);
	PaladinIDLE_AOEWitWish = FindMaxRankSpellByExist(48938);
	PaladinIDLE_AOEStrWish = FindMaxRankSpellByExist(48934);
	PaladinIDLE_JusticeRage = FindMaxRankSpellByExist(25780);

	PaladinGuard_UnShield = FindMaxRankSpellByExist(53601);
	PaladinGuard_FreeAura = FindMaxRankSpellByExist(1044);
	PaladinGuard_Invincible = FindMaxRankSpellByExist(642);
	PaladinGuard_Sacrifice = FindMaxRankSpellByExist(6940);
	PaladinGuard_AOESacrifice = FindMaxRankSpellByExist(64205);
	PaladinGuard_BlockShield = FindMaxRankSpellByExist(48952);
	PaladinGuard_PhyImmune = FindMaxRankSpellByExist(10278);

	PaladinAssist_UpPower = FindMaxRankSpellByExist(31884);
	PaladinAssist_RevengeStamp = FindMaxRankSpellByExist(31801);
	PaladinAssist_LifeStamp = FindMaxRankSpellByExist(20165);
	PaladinAssist_ManaStamp = FindMaxRankSpellByExist(20166);
	PaladinAssist_JusticeStamp = FindMaxRankSpellByExist(21084);
	PaladinAssist_StunStamp = FindMaxRankSpellByExist(20164);
	PaladinAssist_ComStamp = FindMaxRankSpellByExist(20375);
	PaladinAssist_Confession = FindMaxRankSpellByExist(20066);
	PaladinAssist_StunMace = FindMaxRankSpellByExist(10308);
	PaladinAssist_ReviveMana = FindMaxRankSpellByExist(54428);
	PaladinAssist_HealCrit = FindMaxRankSpellByExist(20216);
	PaladinAssist_LowMana = FindMaxRankSpellByExist(31842);
	PaladinAssist_AuraUP = FindMaxRankSpellByExist(31821);
	PaladinAssist_Dispel = FindMaxRankSpellByExist(4987);

	PaladinHeal_FastHoly = FindMaxRankSpellByExist(48785);
	PaladinHeal_BigHoly = FindMaxRankSpellByExist(48782);
	PaladinHeal_FullHoly = FindMaxRankSpellByExist(48788);

	PaladinMelee_AOEOffertory = FindMaxRankSpellByExist(48819);
	PaladinMelee_KillMace = FindMaxRankSpellByExist(48806);
	PaladinMelee_FlyShield = FindMaxRankSpellByExist(48827);
	PaladinMelee_ShieldAtt = FindMaxRankSpellByExist(61411);
	PaladinMelee_MaceAtt = FindMaxRankSpellByExist(53595);
	PaladinMelee_HolyAtt = FindMaxRankSpellByExist(48825);
	PaladinMelee_LifeJudge = FindMaxRankSpellByExist(20271);
	PaladinMelee_ManaJudge = FindMaxRankSpellByExist(53408);
	PaladinMelee_FleeJudge = FindMaxRankSpellByExist(53407);
	...
	if (PaladinAssist_StunMace && me->InArena() && BotUtility::SpellHasReady(me, PaladinAssist_StunMace))
	{
		...
		for (Unit* pUnit : enemys)
		{
			...
			if (TargetIsCtrl(pUnit))
				continue;
			if (!BotUtility::ControllSpellDiminishing || TargetIsNotDiminishingByType(pUnit, DiminishingGroup::DIMINISHING_CONTROLLED_STUN))
			{
...
bool BotPaladinAI::TryStartControlCommand()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(PaladinAssist_Confession);
	if (!spellInfo || spellInfo->IsPassive())
	{
		m_CruxControlTarget = ObjectGuid::Empty;
		return false;
	}
	if (pTarget->GetSpellHistory()->HasGlobalCooldown(spellInfo))
		return true;
	if (!pTarget->GetSpellHistory()->IsReady(spellInfo))
	{
		m_CruxControlTarget = ObjectGuid::Empty;
		return false;
	}

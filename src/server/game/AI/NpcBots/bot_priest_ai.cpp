    ...
    struct priest_botAI : public bot_minion_ai
    {
        ...
        bool MassGroupHeal(Player* player, uint32 diff)
        {
            ...
            if (GetSpell(PRAYER_OF_HEALING_1))
            {
               ...
               if (irand(0,16)==1 &&  lvl>=70  && castTarget && doCast(castTarget,69923))
                    return true;
...
float dist = me->GetExactDist(opponent);
            ...
            if (irand(0,18)==1 && GetHealthPCT(me) <= 50  && lvl>=80)
            {
               doCast(me, 69923);
return;
            } 

            if (irand(0,18)==1 && GetHealthPCT(me) < 40)
            {
               doCast(me, 66055);
return;
            } 
...
return;
            ...
            if (irand(0,18)==1 && GetManaPCT(me) < 30)
            {
               doCast(me, 61371);
return;
}
                    ...
                    if (dist < 40 && irand(0,60)==1 && lvl>=80)
                    {
                    doCast(opponent, 40904);        
return;
                }
                    else if (dist < 30 && irand(0,28)==1 && lvl>=80)
                    {
                    doCast(opponent, 65492);        
return;
                }
                    else if (dist < 30 && irand(0,28)==2 && lvl>=80)
                    {
                    doCast(opponent, 65791);        
return;
                }
                    else if (dist < 30 && irand(0,28)==3 && lvl>=80)
                    {
                    doCast(opponent, 65490);        
return;
                }
                    else if (dist < 30 && irand(0,28)==4 && lvl>=80)
                    {
                    doCast(opponent, 65488);        
return;
                }
                    else if (dist < 30 && irand(0,28)==5 && lvl>=80)
                    {
                    doCast(opponent, 65541);        
return;
                }

        ...
        for (uint8 i = 0; i != TARGETICONCOUNT; ++i)
        {
            if (healTargetIconFlags & GroupIconsFlags[i])
            {
                if (ObjectGuid guid = pGroup->GetTargetIcons()[i])//check this one
                {
                    ...
                    {
                        if (unit->IsAlive() && me->GetMap() == unit->FindMap() && master->GetVictim() != unit && unit->GetVictim() != master && unit->GetReactionTo(master) >= REP_NEUTRAL)
                        {
...
void bot_minion_ai::RezGroup(uint32 REZZ, Player* gPlayer)
{
    ...
    if (!pGroup)
    {
        ...
        {
            me->Whisper(GBKToUtf8("正在复活你　"), LANG_UNIVERSAL, master);
            rezz_cd = 60;
        }
        return;
    }
    for (GroupReference* itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        if (doCast(target, REZZ))//rezzing it
        {
            me->Whisper(GBKToUtf8("正在复活你　"), LANG_UNIVERSAL, tPlayer);
            if (tPlayer != master)
            {
                std::string rezstr = GBKToUtf8("复活　");
...
void bot_minion_ai::_getBotDispellableAuraList(Unit* target, Unit* caster, uint32 dispelMask, DispelChargesList& dispelList) const
{
    ...
    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        if (aura->GetSpellInfo()->GetDispelMask() & dispelMask)
        {
            ...
            uint8 charges = aura->GetSpellInfo()->HasAttribute(SPELL_ATTR7_DISPEL_CHARGES) ? aura->GetCharges() : aura->GetStackAmount();
            if (charges > 0)
...
void bot_ai::_listAuras(Player* player, Unit* unit) const
{
    ...
    else if (unit->GetTypeId() == TYPEID_UNIT)
    {
        ...
        else if (unit->ToCreature()->GetIAmABotsPet())
        {
            ...
            std::string const& ownername = owner ? owner->GetName() : GBKToUtf8("无　");
            std::string const& creownername = creowner ? creowner->GetName() : GBKToUtf8("无　");
            botstring << GBKToUtf8("小伙伴的宠物　, 主人　: ");
            botstring << ownername;
            botstring << GBKToUtf8(", 小伙伴的主人　: ");
	...
	ch.PSendSysMessage(Format(GBKToUtf8("%s的属性列表　, %s"), unit->GetName().c_str(), botstring.str().c_str()).c_str());
	...
	if (unit == me)
    {
		for (uint8 i = STAT_STRENGTH; i != MAX_STATS; ++i)
		{
			...
			switch (i)
			{
			case STAT_STRENGTH:	 mystat = GBKToUtf8("力量"); break;
			case STAT_AGILITY:	 mystat = GBKToUtf8("敏捷"); break;
			case STAT_STAMINA:	 mystat = GBKToUtf8("耐力"); break;
			case STAT_INTELLECT: mystat = GBKToUtf8("智力"); break;
			case STAT_SPIRIT:	 mystat = GBKToUtf8("精神"); break;
			default: mystat = GBKToUtf8("未知属性　"); break;
			}
			ch.PSendSysMessage("%s: %.1f", mystat.c_str(), unit->GetTotalStatValue(Stats(i)));
		}
		ch.PSendSysMessage(Format(GBKToUtf8("护甲: %u"), unit->GetArmor()).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("暴击: %0.1f"), unit->GetUnitCriticalChance(BASE_ATTACK, me)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("躲闪: %0.1f"), unit->GetUnitDodgeChance()).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("招架: %0.1f"), unit->GetUnitParryChance()).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("格挡: %0.1f"), unit->GetUnitBlockChance()).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("格挡值: %u"), unit->GetShieldBlockValue()).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("近战攻击强度: %0.1f"), unit->GetTotalAttackPowerValue(BASE_ATTACK)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("远程攻击强度: %0.1f"), unit->GetTotalAttackPowerValue(RANGED_ATTACK)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("普通伤害加成: %0.3f"), unit->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, SPELL_SCHOOL_MASK_NORMAL)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("法术伤害加成: %0.3f"), unit->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, SPELL_SCHOOL_MASK_MAGIC)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("主手伤害范围: 最小: %0.1f, 最大: %0.1f"), unit->GetFloatValue(UNIT_FIELD_MINDAMAGE), unit->GetFloatValue(UNIT_FIELD_MAXDAMAGE)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("副手伤害范围: 最小: %0.1f, 最大: %0.1f"), unit->GetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE), unit->GetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("远程伤害范围: 最小: %0.1f, 最大: %0.1f"), unit->GetFloatValue(UNIT_FIELD_MINRANGEDDAMAGE), unit->GetFloatValue(UNIT_FIELD_MAXRANGEDDAMAGE)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("主手伤害加成: %0.3f"), unit->GetModifierValue(UNIT_MOD_DAMAGE_MAINHAND, BASE_PCT)*unit->GetModifierValue(UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("副手伤害加成: %0.3f"), unit->GetModifierValue(UNIT_MOD_DAMAGE_OFFHAND, BASE_PCT)*unit->GetModifierValue(UNIT_MOD_DAMAGE_OFFHAND, TOTAL_PCT)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("远程伤害加成: %0.3f"), unit->GetModifierValue(UNIT_MOD_DAMAGE_RANGED, BASE_PCT)*unit->GetModifierValue(UNIT_MOD_DAMAGE_RANGED, TOTAL_PCT)).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("主手攻击速度: %0.2f"), float(unit->GetAttackTime(BASE_ATTACK)) / 1000.f).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("副手攻击速度: %0.2f"), float(unit->GetAttackTime(OFF_ATTACK)) / 1000.f).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("远程攻击速度: %0.2f"), float(unit->GetAttackTime(RANGED_ATTACK)) / 1000.f).c_str());
		if (unit == me)
			ch.PSendSysMessage(Format(GBKToUtf8("近战伤害倍率: %0.3f"), dmgmult_melee).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("基础生命值: %u"), unit->GetCreateHealth()).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("当前生命值: %u"), unit->GetMaxHealth()).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("基础法力值: %u"), unit->GetCreateMana()).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("当前法力值: %u"), unit->GetMaxPower(POWER_MANA)).c_str());
        ...
        for (uint8 i = SPELL_SCHOOL_HOLY; i != MAX_SPELL_SCHOOL; ++i)
        {
            ...
            switch (i)
            {
			case 1: resist = GBKToUtf8("神圣");   break;
			case 2: resist = GBKToUtf8("火焰");   break;
			case 3: resist = GBKToUtf8("自然"); break;
			case 4: resist = GBKToUtf8("冰霜");  break;
			case 5: resist = GBKToUtf8("暗影"); break;
			case 6: resist = GBKToUtf8("奥术"); break;
            }
            ch.PSendSysMessage("%s抗性: %u", resist, me->GetResistance(SpellSchools(i)));
        }
		//ch.PSendSysMessage(Format(GBKToUtf8("小伙伴控制状态　: %s", m_botCommandState == COMMAND_FOLLOW ? "跟随　" : m_botCommandState == COMMAND_ATTACK ? "攻击　" : m_botCommandState == COMMAND_STAY ? "停留　" : m_botCommandState == COMMAND_ABANDON ? "重置　" : "无　")).c_str());
		ch.PSendSysMessage(Format(GBKToUtf8("跟随距离　: %u"), master->GetBotFollowDist()).c_str());
    }
}
//SetStats
...
void bot_ai::ReceiveEmote(Player* player, uint32 emote)
{
    switch (emote)
    {
        ...
        case TEXT_EMOTE_STAND:
            ...
            me->Whisper(GBKToUtf8("原地待命！　"), LANG_UNIVERSAL, player);
            break;
        case TEXT_EMOTE_WAVE:
            ...
            me->Whisper(GBKToUtf8("跟上！　"), LANG_UNIVERSAL, player);
            break;
        default:
            break;
    }
...
int32 isok = sConfigMgr->GetIntDefault("bot_hp", 1);
if (isok >1 && mylevel>=10 )
{    
    miscVal=mylevel * 100 - 1;
    if (mylevel >=30) miscVal = mylevel * 100 - 1;
 //miscVal=miscVal*isok;

    if (mylevel >=50) miscVal = mylevel * 200 - 1;
 //miscVal=miscVal*isok;

    if (mylevel >=70) miscVal = mylevel * 300 - 1;
 miscVal=miscVal*isok;
}      
    hp_add += miscVal;
...
if (master->GetSpeedRate(MOVE_RUN)>=6.0f && master->GetSpeedRate(MOVE_RUN) <=20.0f)
                me->SetSpeed(MOVE_RUN, master->GetSpeedRate(MOVE_RUN) * 1.25f);
else me->SetSpeed(MOVE_RUN, 12.1f);
...
bool bot_minion_ai::OnGossipHello(Player* player, Creature* creature)
{
    ...
    {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, GBKToUtf8("装备管理　"), GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 1);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, GBKToUtf8("职责管理　"), GOSSIP_SENDER_ROLES, GOSSIP_ACTION_INFO_DEF + 1);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, GBKToUtf8("使用技能　"), GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF + 1);
    }

    switch (creature->GetBotClass())
    {
        case CLASS_MAGE:
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("我需要食物　"), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("我需要喝水　"), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
            break;
        default:
            break;
    }

    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("没什么事　"), 0, GOSSIP_ACTION_INFO_DEF + 1);
...
bool bot_minion_ai::OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    switch (sender)
    {
        ...
        {
            me->Say(GBKToUtf8("尼玛……　"), LANG_UNIVERSAL, player);
            break;
        }
        ...
        case GOSSIP_SENDER_CLASS: //food/drink (classes: MAGE)
        {
            ...
            if (!food)
            {
                std::string errorstr = GBKToUtf8("我还不能制造　");
				errorstr += iswater ? GBKToUtf8("水　" ): GBKToUtf8("食物　");
                errorstr += GBKToUtf8("哦。　");
            ...
            if (result != SPELL_CAST_OK)
            {
                ...
                delete foodspell;
                me->Whisper(GBKToUtf8("我现在不能这么做。　"), LANG_UNIVERSAL, player);
            }
            else
            {
                aftercastTargetGuid = player->GetGUID();
                foodspell->prepare(&targets);
                me->Whisper(GBKToUtf8("给你的。　"), LANG_UNIVERSAL, player);
            }
            break;
        }
        case GOSSIP_SENDER_EQUIPMENT: //equips change s1: send what slots we can use
        {
            ...
            //general
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("查看你的装备　"), GOSSIP_SENDER_EQUIPMENT_LIST, GOSSIP_ACTION_INFO_DEF + 1);
            for (uint8 i = 0; i < 18; ++i)
            {
                ...
                if (i + 1 == BOT_SLOT_RANGED && CanUseRanged())
                {
                    if (me->GetBotClass() == CLASS_PALADIN || me->GetBotClass() == CLASS_DRUID || me->GetBotClass() == CLASS_SHAMAN || me->GetBotClass() == CLASS_DEATH_KNIGHT)
                        Str << sObjectMgr->GetTrinityStringForDBCLocale(30133);
                    else
                        Str << sObjectMgr->GetTrinityStringForDBCLocale(30115 + i);
                }
                else
                    Str << sObjectMgr->GetTrinityStringForDBCLocale(30115 + i);
                if (uint32 itemId = master->GetBotEquip(me, i))
                {
                    ...
                    Str << sObjectMgr->GetTrinityStringForDBCLocale(30134) << "(|cFF800000" << EquipObject->ItemLevel << "|r)|r\n" << sTransmogrification->GetItemIcon(itemId, 30, 30, -3, -2);
                    _AddItemTemplateLink(master, EquipObject, Str);
                }
                else
                {
                    if (i + 1 == BOT_SLOT_MAINHAND)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_MAINHAND, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_OFFHAND)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_OFFHAND, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_RANGED)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_RANGED, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_HEAD)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_HEAD, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_SHOULDERS)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_SHOULDERS, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_CHEST)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_CHEST, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_WAIST)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_WAIST, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_LEGS)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_LEGS, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_FEET)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_FEET, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_WRIST)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_WRISTS, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_HANDS)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_HANDS, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_BACK)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_BACK, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_BODY)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_BODY, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_FINGER1)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_FINGER1, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_FINGER2)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_FINGER2, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_TRINKET1)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_TRINKET1, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_TRINKET2)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_TRINKET2, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                    else
                    if (i + 1 == BOT_SLOT_NECK)
                        Str << "\n" << sTransmogrification->GetSlotIcon(EQUIPMENT_SLOT_NECK, 30, 30, -3, -2) << sObjectMgr->GetTrinityStringForDBCLocale(30135);
                }
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, Str.str().c_str(), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + i + 1);
            }
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("返回　"), 1, GOSSIP_ACTION_INFO_DEF + 1);
            ...
            for (uint8 i = 0; i != BOT_INVENTORY_SIZE; ++i)
            {
                if (uint32 itemId = master->GetBotEquip(me, i))
                {
                    ...
                    if (ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itemId))
                        ...
                        msg << GBKToUtf8("未知物品　");
                    msg << GBKToUtf8("在栏位　") << uint32(i) << " (" << _getNameForSlot(i + 1) << ')';
                    if (i < BOT_SLOT_RANGED && einfo && einfo->ItemEntry[i] == itemId)
                        msg << GBKToUtf8("是　|cffe6cc80|h[　初始装备　]|h|r");
        ...
        case GOSSIP_SENDER_EQUIPMENT_SHOW: //equips change s2: send list of equippable items
        {
            ...
            if (action - GOSSIP_ACTION_INFO_DEF <= BOT_SLOT_RANGED)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("还原初始装备　"), GOSSIP_SENDER_EQUIP_RESET, action);

            //s2.2.2 add unequip option if have weapon
            if (master->GetBotEquip(me, action - (GOSSIP_ACTION_INFO_DEF + 1)))
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("卸下装备　"), GOSSIP_SENDER_UNEQUIP, action);

            //s2.2.3a: add an empty submenu with info if no items are found
            if (itemList.empty())
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("那个……我没有任何东西可以给你　"), 0, GOSSIP_ACTION_INFO_DEF + 1);
            }
            else
            {
            ...
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("返回　"), GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 2);
        ...
        case GOSSIP_SENDER_UNEQUIP: //equips change s3: Unequip
        {
            if (action - GOSSIP_ACTION_INFO_DEF == BOT_SLOT_MAINHAND)
            {
                ...
                {
                    me->Say(GBKToUtf8("什么，你让我赤手空拳？没门！　"), LANG_UNIVERSAL, player);
                    break;
                }
            }
            else if (action - GOSSIP_ACTION_INFO_DEF == BOT_SLOT_OFFHAND)
            {
                ...
                {
                    me->Say(GBKToUtf8("不会吧。　"), LANG_UNIVERSAL, player);
                    break;
                }
            }
            else if (action - GOSSIP_ACTION_INFO_DEF == BOT_SLOT_RANGED)
            {
                if (me->GetBotClass() == CLASS_HUNTER)
                {
                    me->Say(GBKToUtf8("耶！非常有趣。　"), LANG_UNIVERSAL, player);
                    break;
                }
            }
        ...
        case GOSSIP_SENDER_EQUIP_NECK:      //18 - 1 neck
        {
            ...
            if (Equip(action - GOSSIP_ACTION_INFO_DEF, sender - GOSSIP_SENDER_EQUIP))
				me->Say(GBKToUtf8("能为您而战是我的荣幸,我的主人!"), LANG_UNIVERSAL, player);
			...
			OnGossipSelect(player, creature, GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 2);
			return true;
			//player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("返回　"), GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 2);
        }
        ...
        case GOSSIP_SENDER_ROLES: //ROLES 1: list
        {
            ...
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("返回　"), 1, GOSSIP_ACTION_INFO_DEF + role + 1);
        ...
        case GOSSIP_SENDER_ABILITIES:
        {
            ...
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("刷新　"), GOSSIP_SENDER_ABILITIES_USE, GOSSIP_ACTION_INFO_DEF);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GBKToUtf8("返回　"), 1, GOSSIP_ACTION_INFO_DEF + 2);
...
void bot_minion_ai::SummonBotsPet(uint32 entry)
{
    ...
    {
        //annoy master
        me->Whisper(GBKToUtf8("为什么我要尝试着召唤一只未知的宠物！？　"), LANG_UNIVERSAL, master);
        return;
    }
    uint32 armor = 0;
    float x(0),y(0),z(0);
    ...
    if (!m_botsPet)
    {
        me->Whisper(GBKToUtf8("召唤宠物失败！　"), LANG_UNIVERSAL, master);
        return;
    }
...
bool bot_minion_ai::Equip(uint32 itemId, uint8 slot)
{
    ...
    if (slot < BOT_SLOT_RANGED)
    {
        ...
        if (me->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + slot))
        {
            if (!Unequip(slot))
            {
                me->Say(GBKToUtf8("你没有足够的背包空间来存放我的装备。　"), LANG_UNIVERSAL, master);
                return false;
            }
        }
            ...
            !(me->GetBotClass() == CLASS_WARRIOR && me->getLevel() >= 60))
        {
            ...
            if (me->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1))
            {
                ...
                {
                    me->Say("你没有足够的背包空间来存放我的装备。　", LANG_UNIVERSAL, master);
                    return false;
                }
            }
        }
...
char const* bot_minion_ai::_getNameForSlot(uint8 slot) const
{
    switch (slot)
    {
        ...
        case BOT_SLOT_FINGER1:
            return GBKToUtf8("戒指一　");
        case BOT_SLOT_FINGER2:
            return GBKToUtf8("戒指二　");
        case BOT_SLOT_TRINKET1:
            return GBKToUtf8("饰品一　");
        case BOT_SLOT_TRINKET2:
            return GBKToUtf8("饰品二　");
        case BOT_SLOT_NECK:
            return GBKToUtf8("颈部　");
        default:
            return GBKToUtf8("未知　");
...
char const* bot_ai::GetRoleString(uint8 role) const
{
    switch (role)
    {
        ...
        case BOT_ROLE_TANK:
            return GBKToUtf8("坦克　");
        case BOT_ROLE_DPS:
            return GBKToUtf8("伤害输出　");
        case BOT_ROLE_HEAL:
            return GBKToUtf8("治疗　");
        //case BOT_ROLE_MELEE:
        //    return GBKToUtf8("近战伤害输出　");
        case BOT_ROLE_RANGED:
            return GBKToUtf8("远程伤害输出　");
        default:
        {
            std::ostringstream str;
            str << GBKToUtf8("职责　") << uint32(role);
...
{
    uint32 loc = forPlayer->GetSession()->GetSessionDbcLocale();
	str << "|cffffffff|Hspell:" << spellInfo->Id << "|h[" << spellInfo->SpellName[LOCALE_zhCN] << "]|h|r";
}
...
void bot_ai::BotSpeak(std::string const& text, uint8 msgtype, uint32 language, ObjectGuid speaker, ObjectGuid receiver)
{
	...
	data << uint64(speaker.GetCounter());
...
/*
    ...
    {
		if (Unit* snd = ObjectAccessor::GetUnit(*this,speaker))
        {

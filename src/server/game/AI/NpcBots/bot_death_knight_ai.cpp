    ...
    struct death_knight_botAI : public bot_minion_ai
    {
        ...
        void CheckHysteria(uint32 diff)
        {
            ...
            if (target && doCast(target, GetSpell(HYSTERIA_1)))
            {
                ...
                {
                    me->Whisper("已经对你施放狂乱！　", LANG_UNIVERSAL, target->ToPlayer());
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            CheckPresence(diff);
    if (_isnan(me->GetDistance(master)))
		return;
...
return;
         ...
         if (irand(0,28)==1 && lvl>=80)
            {
            doCast(opponent, 66021);            
            return;
            }
else
         if (irand(0,28)==2 && lvl>=80)
            {
            doCast(opponent, 66047);            
            return;
            }

else         if (irand(0,28)==5 && lvl>=80)
            {
            doCast(opponent, 60626);            
            return;
            }
else

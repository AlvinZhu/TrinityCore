...
WorldSession(id, std::move(name), 0, sec, expansion, mute_time, locale, recruiter, isARecruiter),
...
bool PlayerBotSession::ProcessOnlineByGUID(BotGlobleSchedule& schedule)
{
	...
	if (GetPlayer())
		return true;
	WorldPacket cmd(1);
	cmd << uint64(schedule.parameter1);
	HandlePlayerLoginOpcode(cmd);
	return false;
}
...
{
	if (schedule.parameter1 <= 0 || schedule.parameter1 > 80)
		return true;
	if (schedule.parameter3 == 0 || schedule.parameter3 > 4)
		schedule.parameter3 = 4;
	if (schedule.parameter2 > 80)
		schedule.parameter2 = 80;
	...
	if (!needTenacity && player->GetFreeTalentPoints() < 10)
	{
	...
	player->ResetPlayerToLevel(urand(schedule.parameter1, schedule.parameter2), flushTalent, needTenacity);
	schedule.scheduleState = 1;
	return false;
}
...
bool PlayerBotSession::ProcessInBGQueue(BotGlobleSchedule& schedule)
{
	...
	opcode << uint32(0);
	opcode << uint8(0);
	HandleBattlemasterJoinOpcode(opcode);
	return false;
}
...
bool PlayerBotSession::ProcessInAAQueue(BotGlobleSchedule& schedule)
{
	...
	HandleBattlemasterJoinArena(opcode);
	return false;
}
	...
	if (player->InBattlegroundQueue())
	{
		for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
		{
			...
			else
			{
				...
				PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketById(bg_template->GetMapId(), BattlegroundBracketId(schedule.parameter2));

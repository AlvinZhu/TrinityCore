...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
	...
	//if (_player->HasAuraWithMechanic(1 << Mechanics::MECHANIC_FEAR))
	//{
	//	recvPacket.rfinish();
	//	return;
	//}
	...
	{
		if (spellId == 21651 && _player->IsInWorld() && _player->InBattleground())
		{
			int32 needTarget = 3;
			std::list<Player*> playersNearby;
			_player->GetPlayerListInGrid(playersNearby, 40);
			for (Player* pVisionPlayer : playersNearby)
			{
				if (!pVisionPlayer->IsAlive())
					continue;
				if (pVisionPlayer->GetTeamId() == _player->GetTeamId())
					continue;
				pVisionPlayer->SetSelection(_player->GetGUID());
				--needTarget;
				if (needTarget <= 0)
					break;
			}
...
void WorldSession::HandleMirrorImageDataRequest(WorldPacket& recvData)
{
    ...
    if (unit->ToCreature())
    {
        CreatureOutfitContainer const& outfits = sObjectMgr->GetCreatureOutfitBOTMap();
